...
export default function CollectionsPage() {
  ...
  return (
    <div className="min-h-screen pb-24">
      <div className="container-normal mx-auto py-16 px-4 space-y-24">
          ...
          {collections.map((collection, index) => (
            <div key={index} className="space-y-12">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div className={cn("relative aspect-square lg:aspect-[4/3] rounded-[3rem] overflow-hidden shadow-2xl border-4 border-white", index % 2 !== 0 && "lg:order-last")}>
                  <Image src={collection.image} alt={collection.title} fill className="object-cover" />
                </div>
                <div className="space-y-6">
                  <h2 className="text-4xl lg:text-6xl font-black uppercase tracking-tight text-foreground">{collection.title}</h2>
                  <p className="text-foreground/60 text-lg lg:text-xl font-light leading-relaxed">{collection.description}</p>
                  <Link href="/products">
                    <button className="flex items-center gap-2 text-primary font-black uppercase tracking-widest text-xs group mt-4">
                      Explore All Pieces
              ...
              <div className="grid grid-cols-3 gap-4 lg:gap-12">
                {collection.subCategories.map((sub, sIdx) => (
                  <Link key={sIdx} href={sub.href} className="group text-center space-y-4">
                    ...
                    <p className="text-[10px] lg:text-sm font-black uppercase tracking-widest text-foreground/70 group-hover:text-primary transition-colors">
                      {sub.name}
                    </p>
                  </Link>
                ))}
